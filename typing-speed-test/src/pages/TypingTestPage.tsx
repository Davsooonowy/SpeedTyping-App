...
import {ACCURACY_DESCRIPTION, CPM_DESCRIPTION, TIMER_DESCRIPTION, WPM_DESCRIPTION} from '../assets/metricsDescriptions';
    ...
    return (
        <Container style={{
            ...
            <Grid container spacing={2} justifyContent="center">
                <Grid item><StatsCircle label="Time Left" value={`${timeLeft}s`}
                                        description={TIMER_DESCRIPTION}/></Grid>
                ...
                <Grid item><StatsCircle label="Accuracy" value={`${accuracy}%`}
                                        description={ACCURACY_DESCRIPTION}/></Grid>
            </Grid>
            <Box my={4} display="flex" flexDirection="column" alignItems="center" gap={2}>
                <RandomTextPlaceholderInput onInputChange={handleInputChange} input={input} onKeyDown={handleKeyDown}
                                            onUpdateAccuracy={setAccuracy} onUpdateCPM={setCPM} timeLeft={timeLeft}
                                            onUpdateWPM={setWPM} resetTrigger={resetCount}/>
                <OnScreenKeyboard input={input}/>
            </Box>
            <Popup open={showPopup} onClose={() => setShowPopup(false)} onReset={resetTest} wpm={wpm} cpm={cpm}
                   accuracy={accuracy}/>
        </Container>
    );
};
